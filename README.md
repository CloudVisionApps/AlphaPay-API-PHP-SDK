...
PHP SDK module for working with the AlphaPay API
...
```bash
$ composer require alphapay/api-php-sdk
```

## Documentation

**Methods for working with api**: https://doc.alphapay.bg/ <br>

## Authorization

`PAYOUT_KEY` or `PAYMENT_KEY`, also `MERCHANT_UUID` required to use SDK details in [documentation](https://doc.alphapay.bg/general/request).
...
$payment = \AlphaPay\Api\Client::payment(PAYMENT_KEY, MERCHANT_UUID);
...
$payout = \AlphaPay\Api\Client::payout(PAYOUT_KEY, MERCHANT_UUID);
...
array(14) {
  ...
  string(5) "check"
  ["url"]=>
  string(66) "https://pay.alphadex.bg/pay/8b03432e-385b-4670-8d06-064591096795"
  ["expired_at"]=>
  int(1650980953)
  ["status"]=>
  string(5) "check"
...
array(2) {
  ...
  array(1) {
    ...
    array(15) {
      ...
      string(4) "paid"
      ["url"]=>
      string(66) "https://pay.alphadex.bg/pay/87094a43-5fe4-4629-b2fd-c37e8e2af76c"
      ["expired_at"]=>
      int(1650960209)
      ["status"]=>
      string(4) "paid"
...
$payment = \AlphaPay\Api\Client::payment(PAYOUT_KEY, MERCHANT_UUID);

try {
    $result = $payment->services();
} catch (\AlphaPay\Api\RequestBuilderException $e) {
    log('Error request AlphaPay to method ' . $e->getMethod() . ': ' . $e->getMessage());
}
